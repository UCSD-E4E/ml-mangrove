...
class ResNet_UNet(Module):
    ...
    def forward(self, image, timestep=None):
        ...
        if timestep:
            # Encode with timestep embeddings
            x1 = self.add_timestep_embedding(self.layer1(image), timestep)
            x2 = self.add_timestep_embedding(self.layer2(x1), timestep)
            x3 = self.add_timestep_embedding(self.layer3(x2), timestep)
            x4 = self.add_timestep_embedding(self.layer4(x3), timestep)
        else:
            # Encode
...
class ResNet_UNet_Diffusion(Module):
    def __init__(self, unet: Optional[ResNet_UNet] = None, num_timesteps=1000, opt=None):
